...
/*
 * The single source of truth for the Montarro "Book a Strategy Call" enquiry
 * form. Rendered identically on the /contact page (primary destination) and at
 * the bottom of the homepage (secondary conversion). Same fields, same styling,
 * same GHL webhook + calendar workflow — change it here and both update.
 */

// Generic lead webhook (public — handled by Make/Zapier/GHL automation
// separately). Referenced as import.meta.env.VITE_* so Vite inlines it at build.
...
export function StrategyCallForm({ className = "" }: { className?: string }) {
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ background: "radial-gradient(ellipse 60% 50% at 50% 0%, rgba(16,185,129,0.10), transparent 70%)" }}
      />
      <div
        aria-hidden
                  ...
                  <div className="mt-10">
                    {step === 1 && (
                      <div className="grid gap-5 sm:grid-cols-2">
                        <Field id="company" label="Company / Brand" value={form.company} onChange={(v) => update("company", v)} error={errors.company} autoComplete="organization" />
                        <Field id="fullName" label="Contact Name" value={form.fullName} onChange={(v) => update("fullName", v)} error={errors.fullName} autoComplete="name" />
                        <Field id="email" label="Work Email" type="email" value={form.email} onChange={(v) => update("email", v)} error={errors.email} autoComplete="email" inputMode="email" />
                        <Field id="phone" label="Phone Number" type="tel" value={form.phone} onChange={(v) => update("phone", v)} error={errors.phone} autoComplete="tel" inputMode="tel" />
                      </div>
                    )}

                    {step === 2 && (
                      <ChoiceGrid options={INDUSTRIES} value={form.industry} onChange={(v) => update("industry", v)} error={errors.industry} columns={2} />
                    )}
                    {step === 3 && (
                      <ChoiceGrid options={STAGES} value={form.stage} onChange={(v) => update("stage", v)} error={errors.stage} columns={1} />
                    )}
                    {step === 4 && (
                      <ChoiceGrid options={GOALS} value={form.goal} onChange={(v) => update("goal", v)} error={errors.goal} columns={2} />
                    )}
                    {step === 5 && (
                      <ChoiceGrid options={BUDGETS} value={form.budget} onChange={(v) => update("budget", v)} error={errors.budget} columns={1} />
                    )}
                    {step === 6 && (
      ...
      {/* contact alternates — only on the form step */}
      {!submitted && (
...
// Public calendar embed URL (no secret). VITE_GHL_CALENDAR_URL may hold EITHER a
// bare booking URL OR a full GHL embed snippet — we extract the booking src.
...
/**
 * Final stage: after the form submits to GoHighLevel the user books a time on
 * the GHL calendar without leaving the page. (Remove the calendar here and it is
 * removed everywhere this form is used.)
 */
function BookingScreen({ lead }: { lead: FormState }) {
  ...
  return (
    ...
    >
      ...
      <p className="text-[11px] uppercase tracking-[0.3em] text-emerald-700/80">One Final Step</p>
      ...
      <div className="relative mx-auto mt-6 max-w-xl">
        ...
        <div className="relative overflow-hidden rounded-2xl border border-emerald-500/20 bg-gradient-to-b from-white/90 to-[#f3f4f6]/70 backdrop-blur-xl shadow-[0_1px_0_0_rgba(255,255,255,0.85)_inset,0_36px_90px_-50px_rgba(0,0,0,0.22)]">
          <div
            ...
            style={{ background: "linear-gradient(to right, transparent, rgba(16,185,129,0.55), transparent)" }}
          />
          {calendarSrc ? (
            <iframe
        ...
        <div className="mt-6 flex flex-col items-center gap-2.5">
          <h3 className="font-display text-xl tracking-tight text-foreground">Prefer We Call You?</h3>
          ...
          <p className="mt-1 max-w-md text-[13px] text-muted-foreground leading-relaxed">
            A member of our team will contact you directly to discuss your business, answer any questions and determine the best next step.
          </p>
        </div>
      </div>
    </motion.div>
...
function BookingConfirmation() {
  const steps = [
    ...
    ["Strategy Session", "We identify bottlenecks, missed revenue opportunities and automation potential."],
  ...
  return (
    ...
    >
      ...
      <p className="mt-8 text-[11px] uppercase tracking-[0.32em] text-emerald-700/80">Free Consultation Booked</p>
      <h2 className="mt-3 font-display text-gradient-chrome leading-[1.05] tracking-[-0.035em]">
        ...
        <span className="block text-[2.4rem] md:text-[3.25rem] font-semibold tracking-[-0.04em] leading-[0.98]">Free Consultation</span>
      ...
      <p className="mt-6 max-w-md text-[14.5px] text-muted-foreground leading-relaxed">
        Our team will review your current lead flow, missed opportunities, and growth infrastructure before the call so we can provide tailored recommendations specific to your business.
      </p>

      <div className="mt-12 w-full max-w-lg text-left">
        <p className="text-center text-[11px] uppercase tracking-[0.3em] text-emerald-700/80">What Happens Next?</p>
      ...
      <div className="mt-12 flex flex-col items-center gap-5">
        ...
        <p className="text-[12px] text-muted-foreground/70">
          Need assistance?{" "}
          <a href="mailto:team@montarro.com" className="text-emerald-700 transition-colors hover:text-emerald-600">
            team@montarro.com
          </a>
        </p>
      </div>
  ...
  return (
    ...
    >
      ...
      <p className="mt-5 max-w-md text-[14.5px] text-muted-foreground leading-relaxed">
        Your details are in — a Montarro strategist will reach out shortly to set up your consultation.
      </p>
...
function ChoiceGrid({
  ...
  return (
    <div>
      <div className={`grid gap-3 ${columns === 2 ? "sm:grid-cols-2" : "sm:grid-cols-1"}`}>
        {options.map((opt) => {
            ...
            >
              ...
              <span
                className={`relative flex h-4 w-4 items-center justify-center rounded-full border transition-all duration-500 ${
                  selected ? "border-emerald-500 bg-emerald-500" : "border-black/[0.15] group-hover:border-emerald-500/50"
                }`}
              >
      ...
      {error && <p className="mt-4 text-[11px] text-muted-foreground">{error}</p>}
    </div>
  );
}
